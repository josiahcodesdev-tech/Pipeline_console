...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (body.action === 'index') {
      ...
      const rows = []
      for (const [index, part] of chunks(content).entries()) rows.push({user_id:user.id,source_type:sourceType,source_id:sourceId,title,content:part,metadata:{chunk:index},embedding:await embedding(part)})

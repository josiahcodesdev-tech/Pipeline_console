...
/**
 * A secret, with the shell's punctuation taken back off.
 *
 * `supabase secrets set KEY="sk-ant-..."` in PowerShell stores the quotes as
 * part of the value — cmd and PowerShell do not strip them the way a POSIX
 * shell does. The result is a credential that looks right in every listing and
 * is rejected with a bare 401: present, correct, and one character wrong at
 * each end. Matched pairs only, so a value that genuinely begins with a quote
 * is left to fail loudly rather than be quietly altered.
 */
const env = (name: string) => {
  const raw = Deno.env.get(name)?.trim() ?? ''
  const quoted = raw.length > 1 && (
...
function claude(): Anthropic {
  ...
  // Checked here rather than left to come back as a 401 a round trip later.
  // Every Anthropic key begins "sk-ant-", so a value that does not is one that
  // was captured wrong — quotes kept by the shell, a truncated paste, or a key
  // belonging to another provider.
  if (!key.startsWith('sk-ant-')) {
    throw new ServiceError('ANTHROPIC_API_KEY does not look like an Anthropic key — they begin "sk-ant-". Check for quotes or a partial paste captured when the secret was set.')
  }
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (typeof status === 'number') {
      ...
      // "Refused" is the wrong word for most of these and sent a whole round of
      // debugging looking for a policy decision. Each status means something
      // different and points at a different fix, so each says so.
      if (status === 401 || status === 403) {
        return json({error:'Anthropic rejected the API key. It is set on this function but not accepted — check it has not been revoked, and that the value was captured whole when the secret was set.'},502)
      }
      if (status === 429) {
